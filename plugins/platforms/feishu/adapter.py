...
class FeishuAdapter(BasePlatformAdapter):
    ...
    async def _archive_inbound_message(self, *, message: Any, sender: Any, mentioned_bot: bool) -> None:
        """Persist every delivered Feishu message and its media before agent admission."""
        message_id = str(getattr(message, "message_id", "") or "")
        if not message_id:
            return
        sender_id = getattr(sender, "sender_id", None)
        archive_path = get_hermes_home() / "feishu_messages.db"
        archive_dir = get_hermes_home() / "feishu_attachments" / message_id
        ...
        attachments: List[tuple[int, str, str, str, str, str, int, str, str]] = []
        ...
        async def archive_file(
            ...
            try:
                if resource_type == "image":
                    cached_path, mime_type = await self._download_feishu_image(
                        message_id=message_id,
                        image_key=file_key,
                    )
                else:
                    cached_path, mime_type = await self._download_feishu_message_resource(
                        message_id=message_id,
                        file_key=file_key,
                        resource_type=resource_type,
                        fallback_filename=original_name,
                    )
                if not cached_path:
            ...
            if cached_path:
                try:
                    ...
                    safe_name = Path(original_name or Path(cached_path).name).name
                    if not safe_name or safe_name in {".", ".."}:
                        safe_name = Path(cached_path).name
                    destination = archive_dir / f"{index:03d}_{safe_name}"
                    ...
                    error = str(exc)
            attachments.append(
                (index, file_key, resource_type, original_name, mime_type, local_path, file_size, status, error)
            )

        attachment_index = 0
        for image_key in normalized.image_keys:
            await archive_file(attachment_index, image_key, "image", "")
            attachment_index += 1
        for media_ref in normalized.media_refs:
            await archive_file(
                attachment_index,
                media_ref.file_key,
                media_ref.resource_type,
                media_ref.file_name,
            )
            attachment_index += 1

        try:
            with sqlite3.connect(archive_path, timeout=5) as conn:
                conn.execute(
                    """
                ...
                conn.execute(
                    ...
                    "ON feishu_messages(chat_id, received_at)"
                )
                conn.execute(
                    """
                ...
                conn.execute(
                    "CREATE INDEX IF NOT EXISTS idx_feishu_attachments_message "
                    "ON feishu_attachments(message_id)"
                )
                conn.execute(
                    """
                    INSERT OR IGNORE INTO feishu_messages (
                        message_id, received_at, create_time, chat_id, chat_type,
                        sender_open_id, sender_user_id, sender_union_id, message_type,
                        content, mentioned_bot
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        message_id,
                        ...
                        str(getattr(message, "chat_type", "p2p") or "p2p"),
                ...
                conn.executemany(
                    """
                    INSERT OR REPLACE INTO feishu_attachments (
                        message_id, attachment_index, file_key, resource_type,
                        original_name, mime_type, local_path, file_size,
                        download_status, download_error
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    [(message_id, *attachment) for attachment in attachments],
                )
    ...
    async def _handle_message_event_data(self, data: Any) -> None:
        ...
        mentioned_bot = not is_group or self._mentions_self(message)
        await self._archive_inbound_message(
            message=message,
            sender=sender,
            mentioned_bot=mentioned_bot,
        )
        if is_group and not mentioned_bot:
            logger.info(
                "[Feishu] Archived unmentioned group message: id=%s chat_id=%s",
                message_id,
